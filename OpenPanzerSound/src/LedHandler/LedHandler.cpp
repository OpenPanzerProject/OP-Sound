...
void OPS_LedHandler::begin(byte p, boolean i /*=false*/)
{
...
void OPS_LedHandler::pinOn(void)
{
    _invert ? digitalWrite(_pin, LOW) : digitalWrite(_pin, HIGH);
}

void OPS_LedHandler::pinOff(void)
{
    _invert ? digitalWrite(_pin, HIGH) : digitalWrite(_pin, LOW);
}

void OPS_LedHandler::toggle(void)
{
    digitalWrite(_pin, !digitalRead(_pin)); 
}
    
boolean OPS_LedHandler::isBlinking(void)
{
    return _isBlinking; 
}

void OPS_LedHandler::Blink(uint16_t interval /*=DEFAULT_BLINK_INTERVAL*/)
{
...
void OPS_LedHandler::stopBlinking(void)
{
    this->pinOff();
    ClearBlinker();
}

void OPS_LedHandler::startBlinking(uint16_t on_interval, uint16_t off_interval)
{
    BlinkStream Blinker;    
...
void OPS_LedHandler::blinkHeartBeat(void)
{
    this->startBlinking(BLINK_RATE_HEARTBEAT, BLINK_RATE_HEARTBEAT);
}

void OPS_LedHandler::blinkLostSignal(void)
{
    this->startBlinking(BLINK_RATE_LOST_SIGNAL, BLINK_RATE_LOST_SIGNAL);
}

void OPS_LedHandler::DoubleTap(boolean repeat /*=false*/)
{
    BlinkStream bs;
...
void OPS_LedHandler::TripleTap(boolean repeat /*=false*/)
{
    BlinkStream bs;
...
void OPS_LedHandler::StreamBlink(BlinkStream bs, uint8_t numSteps)
{
    ClearBlinker();
...
void OPS_LedHandler::ClearBlinker(void)
{
...
void OPS_LedHandler::ExpireIn(uint16_t expireTime)
{
    _expireTime = expireTime;
    _expireMe = 0;
}

void OPS_LedHandler::update(void)
{
